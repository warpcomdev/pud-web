...
class GaviasPageBuilderController extends ControllerBase {
  ...
  public function gavias_pagebuilder_edit($bid) {
  	gavias_pagebuilder_init();
  	
    require_once \Drupal::service('extension.list.module')->getPath('gavias_pagebuilder') . '/includes/utilities.php';
	      	...
	      	if ($node instanceof \Drupal\node\NodeInterface) {
	        	try {
	        		if(count($languagesAll) > 1){
	          		$data = $node->getTranslation($language)->get('gva_pagebuilder_content')->value;
	          		$title = 'pagebuilder-' . $language . '-' . $this->friendly_title($node->get('title')->value); 
	          	}else{
